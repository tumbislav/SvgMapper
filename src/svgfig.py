...
if re.search("windows", platform.system(), re.I):
    try:
        ...
        _default_directory = _winreg.QueryValueEx(_winreg.OpenKey(
            _winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\Current Version\Explorer\Shell Folders"),
                                                  "Desktop")[0]
...
def rgb(r, g, b, maximum=1.):
    ...
    return "#%02x%02x%02x" % (max(0, min(r * 255. / maximum, 255)),
                              max(0, min(g * 255. / maximum, 255)), max(0, min(b * 255. / maximum, 255)))
...
class SVG:
    ...
    class SVGDepthIterator:
        ...
        def next(self):
            ...
            if self.depth_limit is not None and len(self.ti) >= self.depth_limit: raise StopIteration
...
_canvas_defaults = {"width": "400px", "height": "400px", "viewBox": "0 0 100 100", \
                    ...
                    "version": "1.1", \
                    "style": {"stroke": "black", "fill": "none", "stroke-width": "0.5pt", "stroke-linejoin": "round",
                              "text-anchor": "middle"}, \
...
def canvas_outline(*sub, **attr):
    ...
    svg = canvas(*sub, **attr)
    match = re.match("[, \t]*([0-9e.+\-]+)[, \t]+([0-9e.+\-]+)[, \t]+([0-9e.+\-]+)[, \t]+([0-9e.+\-]+)[, \t]*",
                     svg["viewBox"])
...
def load_stream(stream):
    ...
    class ContentHandler(handler.ContentHandler):
        ...
        def endElement(self, name):
            if len(self.stack) > 0:
                last = self.stack[-1]
                if isinstance(last, SVG) and last.t == "style" and "type" in last.attr and last.attr[
                    "type"] == "text/css" and len(last.sub) == 1 and isinstance(last.sub[0], basestring):
                    last.sub[0] = "<![CDATA[\n" + last.sub[0] + "]]>"
...
def window(xmin, xmax, ymin, ymax, x=0, y=0, width=100, height=100, xlogbase=None, ylogbase=None, minusInfinity=-1000,
    ...
    if xlogbase != None and (
            ix1 <= 0. or ix2 <= 0.): raise ValueError, "x range incompatible with log scaling: (%g, %g)" % (ix1, ix2)

    if ylogbase != None and (
            iy1 <= 0. or iy2 <= 0.): raise ValueError, "y range incompatible with log scaling: (%g, %g)" % (iy1, iy2)
...
class Plot:
    ...
    axis_attr       {}            a dictionary of attributes for the axis to
    """
    ...
    def __init__(self, xmin, xmax, ymin, ymax, *d, **kwds):
        ...
        self.d = list(d)
        defaults = {"trans": None, "x": 5, "y": 5, "width": 90, "height": 90, "flipx": False, "flipy": True,
                    "minusInfinity": -1000,
    ...
    def SVG(self, trans=None):
        ...
        self.last_window = window(self.xmin, self.xmax, self.ymin, self.ymax, x=self.x, y=self.y, width=self.width,
                                  height=self.height,
                                  xlogbase=self.xlogbase, ylogbase=self.ylogbase, minusInfinity=self.minusInfinity,
        ...
        d = [Axes(self.xmin, self.xmax, self.ymin, self.ymax, self.atx, self.aty,
                  ...
                  self.arrows, self.text_attr, **self.axis_attr)] + self.d
...
class Frame:
    ...
    def __init__(self, xmin, xmax, ymin, ymax, *d, **kwds):
        ...
        defaults = {"x": 20, "y": 5, "width": 75, "height": 80, "flipx": False, "flipy": True, "minusInfinity": -1000,
                    "xtitle": None, "xticks": -10, "xminiticks": True, "xlabels": True, "x2labels": None,
                    "xlogbase": None,
                    "ytitle": None, "yticks": -10, "yminiticks": True, "ylabels": True, "y2labels": None,
                    "ylogbase": None,
                    "text_attr": {}, "axis_attr": {}}
        defaults.update(kwds)
        kwds = defaults
    ...
    def SVG(self):
        ...
        left = YAxis(self.ymin, self.ymax, self.xmin, self.yticks, self.yminiticks, self.ylabels, self.ylogbase, None,
                     None, None, self.text_attr, **self.axis_attr)
        right = YAxis(self.ymin, self.ymax, self.xmax, self.yticks, self.yminiticks, self.y2labels, self.ylogbase, None,
                      None, None, self.text_attr, **self.axis_attr)
        bottom = XAxis(self.xmin, self.xmax, self.ymin, self.xticks, self.xminiticks, self.xlabels, self.xlogbase, None,
                       None, None, self.text_attr, **self.axis_attr)
        top = XAxis(self.xmin, self.xmax, self.ymax, self.xticks, self.xminiticks, self.x2labels, self.xlogbase, None,
                    None, None, self.text_attr, **self.axis_attr)
...
class Path:
    ...
    def parse_number(self, index, pathdata):
        ...
        # FIXED MČ: original lexer read '3.141-2.718' as a single float. SVG specification is very liberal about
        # whitespace, and this sequence is actually two floats
        if "0" <= first_digit <= "9" or first_digit in ("-", "+", "."):
            start = index
            index += 1
            while index < len(pathdata) and ("0" <= pathdata[index] <= "9" or pathdata[index] in (".", "e", "E")):
                if pathdata[index] in ("e", "E"):
                    index += 1
                    if index < len(pathdata) and (pathdata[index] in ("+", "-")):
                        index +=1
                else:
                    index += 1
            end = index
    ...
    def parse(self, pathdata):
        ...
        while True:
            ...
            ######################
                ...
                # FIXED MČ: SVG standard states that an "M" or an "m" followed by multiple pairs of values
                # is to be understood as a moveto followed by a sequence of lineto commands, rather than a
...
class Curve:
    ...
    def subsample(self, left, right, depth, trans=None):
        ...
        if depth < 3 or (denom == 0 and left.t != right.t) or denom > self.discontinuity_limit or (
...
class Poly:
    ...
    mode                    default="L"   "to", "bezier", "velocity",
    ...
    "velocity"/"V"      d=[(x, y, vx, vy), ...]
                                            ...
                                            (0,0), you get a linear interpolation ("to").
    ...
    def Path(self, trans=None, local=False):
        ...
        if self.mode[0] == "L" or self.mode[0] == "l":
            ...
            raise ValueError, "mode must be \"to\", \"bezier\", \"velocity\", \"foreback\", \"smooth\", or an abbreviation"
...
class Ticks:
    ...
    def __init__(self, f, low, high, ticks=-10, miniticks=True, labels=True, logbase=None, arrow_start=None,
                 arrow_end=None, text_attr={}, **attr):
        self.f = f
        self.low = low
        self.high = high
        self.ticks = ticks
...
class LineAxis(Line, Ticks):
    ...
    def __init__(self, x1, y1, x2, y2, start=0., end=1., ticks=-10, miniticks=True, labels=True, logbase=None,
...
class XAxis(LineAxis):
    ...
    text_attr               default={}      SVG attributes for the text labels
    attribute=value pairs   keyword list    SVG attributes for all to
    ...
    def __init__(self, xmin, xmax, aty=0, ticks=-10, miniticks=True, labels=True, logbase=None, arrow_start=None,
                 arrow_end=None, exclude=None, text_attr={}, **attr):
        self.aty = aty
        tattr = dict(self.text_defaults)
        tattr.update(text_attr)
...
class YAxis(LineAxis):
    ...
    def __init__(self, ymin, ymax, atx=0, ticks=-10, miniticks=True, labels=True, logbase=None, arrow_start=None,
                 arrow_end=None, exclude=None, text_attr={}, **attr):
        self.atx = atx
        tattr = dict(self.text_defaults)
        tattr.update(text_attr)
...
class Axes:
    ...
    def __init__(self, xmin, xmax, ymin, ymax, atx=0, aty=0, xticks=-10, xminiticks=True, xlabels=True, xlogbase=None,
                 yticks=-10, yminiticks=True, ylabels=True, ylogbase=None, arrows=None, text_attr={}, **attr):
...
class HGrid(Ticks):
    """Draws the horizontal to of a grid over a specified region
    ...
    logbase                 default=None    if a number, the axis is logarithmic
                                            with ticks at the given base (usually 10)
    mini_attr               default={}      SVG attributes for the minitick-to
                                            (if miniticks != False)
    attribute=value pairs   keyword list    SVG attributes for the major tick to
    """
...
class VGrid(Ticks):
    """Draws the vertical to of a grid over a specified region
    using the standard tick specification (see help(Ticks)) to place the
    grid to.
...
class Grid(Ticks):
    ...
    specification (see help(Ticks)) to place the grid to.
